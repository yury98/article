...
//                    System.out.println(y);
    ...
    private static List<Data> search(String bank, Integer destination, Integer check, Boolean isReverse) throws SQLException {
        ...
        if (check != null) {
            st = conn.prepareStatement("SELECT * FROM CHECKS WHERE sourceBank = ? and sourceAccount = ? and sourceCheck = ? and isReverse = ?;");
            st.setString(1, bank);
            st.setInt(2, destination);
            st.setInt(3, check);
            st.setBoolean(4, isReverse);
        } else {
            st = conn.prepareStatement("SELECT * FROM CHECKS WHERE sourceBank = ? and sourceAccount = ? and isReverse = ?;");
            st.setString(1, bank);
            st.setInt(2, destination);
