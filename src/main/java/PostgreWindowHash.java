...
public class PostgreWindowHash {
...
//                    System.out.println(y);
    ...
    private static List<Data> search(String bank, Integer destination, Integer check, Boolean isReverse, String date) throws SQLException, ParseException {
        PreparedStatement st = conn.prepareStatement("SELECT * FROM CHECKS WHERE hash = ? AND ? - timestamp < INTERVAL '6 hours';");
        if (check != null) {
        ...
        st.setTimestamp(2, new Timestamp(parsedDate.getTime()));
